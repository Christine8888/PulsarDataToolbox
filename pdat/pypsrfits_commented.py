...
class PyPSRFITS:
    ...
    def open(self,fname):
        ...
        as self.subhdr."""
        self.fits = fitsio.FITS(fname,'r') # [fitsio]
        self.hdr = self.fits[0].read_header() # [fitsio]
        self.subhdr = self.fits['SUBINT'].read_header() # [fitsio]

    def get_freqs(self,row=0):
        """Return the frequency array from the specified subint."""
        return self.fits['SUBINT']['DAT_FREQ'][row] # I think this would work for any FITS reader
    ...
    def get_data(self, start_row=0, end_row=None,
        ...
        nsblk = self.subhdr['NSBLK'] # samples in one subint or row
        ...
        nsblk_ds = nsblk / downsamp # is this integer division
        nchan_ds = nchan / fdownsamp
        tbin_ds = tbin * downsamp # how long a time bin should be
