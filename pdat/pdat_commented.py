...
class psrfits(F.FITS):
    ...
    def __init__(self, psrfits_path, mode='rw', from_template=False,
        ...
        # or (from_template==False and mode='rw')
            ...
            self.written = False
            self.fits_template = F.FITS(template_path, mode='r') # [fitsio] creates FITS object

            if self.obs_mode is None:
                OBS = self.fits_template[0].read_header()['OBS_MODE'].strip() # [fitsio] get OBS_MODE from template if not yet set
                self.obs_mode = OBS
            else:
                self.obs_mode = obs_mode
            ...
            self.draft_hdrs['PRIMARY'] = self.fits_template[0].read_header() # [fitsio] start filling out draft_hdrs
            self.n_hdrs = len(self.fits_template.hdu_list) # [fitsio] one header for each HDU

            for ii in np.arange(1,self.n_hdrs): # for each HDU/header
                hdr_key = self.fits_template[ii].get_extname() # [fitsio] get name
                self.draft_hdrs[hdr_key] = self.fits_template[ii].read_header() # [fitsio] set up header draft
        ...
        # super deals w/ inheritance [fitsio]
        if sys.version_info[0]<3:
            ...
            super().__init__(psrfits_path, mode = mode) # [fitsio] initialize a parent class object (FITS)?
        ...
        if self.obs_mode is None and from_template: # i think this happens twice? maybe?
            OBS = self.fits_template[0].read_header()['OBS_MODE'].strip() # [fitsio] get obs_mode from template
            self.obs_mode = OBS
        ...
        if from_template and verbose:
            ...
            self.HDU_drafts = {}
            #Set the ImageHDU to be called primary.
            self.draft_hdrs['PRIMARY'] = self[0].read_header() # [fitsio]
            self.n_hdrs = len(self.hdu_list)
            self.written = False
            for ii in range(self.n_hdrs-1):
                hdr_key = self[ii+1].get_extname() # [fitsio]
                self.draft_hdrs[hdr_key] = self[ii+1].read_header() # [fitsio]
                self.HDU_drafts[hdr_key] = None # [fitsio]
    ...
    def write_psrfits(self, HDUs=None, hdr_from_draft=True):
        ...
        if self.written:
            raise ValueError('PSRFITS file has already been written. '
                             'Can not write twice.')
        if HDUs is None:  # set to draft HDUs
            HDUs = self.HDU_drafts

        if any([val is None for val in HDUs.values()]): # [fitsio] HDUs.values
    ...
    def append_from_file(self,path,table='all'):
        ...
        """
        PF2A = F.FITS(path, mode='r') # [fitsio] create new FITS
        PF2A_hdrs = []
        ...
        for hdr in self.draft_hdr_keys[1:]: # for each non-PRIMARY header
            rec_array = PF2A[list_arg(table,hdr)].read() # [fitsio] read HDU at certain index
            self[list_arg(self.draft_hdr_keys,hdr)].append(rec_array) # [fitsio] append data

#######Convenience Functions################
    def get_colnames():
        """Returns the names of all of the columns of data needed for a PSRFITS
        file."""
        return self[1].get_colnames() # [fitsio]
    ...
    def set_hdr_from_draft(self, hdr):
        ...
        for card in self.draft_hdrs[hdr_name].records(): # [fitsio] for each card in records
    ...
    def get_FITS_card_dict(self, hdr, name):
        ...
        """
        card = next((item for item in hdr.records() # [fitsio] find item in records of header
    ...
    def replace_FITS_Record(self, hdr, name, new_value):
        ...
        #
        #     new_record = self.make_FITS_card(hdr,name,new_value)
        if not isinstance(hdr,F.fitslib.FITSHDR): # [fitsio]
            hdr = self.draft_hdrs[hdr]
             #Maybe faster if try: except: used?
        new_record = self.make_FITS_card(hdr,name,new_value)
        hdr.add_record(new_record) # [fitsio]
    ...
    def get_HDU_dtypes(self, HDU):
        ...
        HDU = A FITS HDU.
        """
        return HDU.get_rec_dtype()[0].descr # [fitsio]
    ...
    def set_HDU_array_shape_and_dtype(self, HDU_dtype_list, name,
        ...
        """
        try:
            ii = [x for x, y in enumerate(HDU_dtype_list) # get first one that matches
                  if y[0] == name.upper()][0]
        except:
    ...
    def write_PrimaryHDU_info_dict(self, ImHDU_template, new_ImHDU):
        ...
        """
        templ_info_keys = list(ImHDU_template.__dict__['_info'].keys()) # [fitsio]
        new_info_keys = list(new_ImHDU.__dict__['_info'].keys()) # [fitsio]
        info_keys = np.unique(np.concatenate((templ_info_keys,new_info_keys))) # get all unique keys needed

        for key in info_keys:
            if key in templ_info_keys:
                new_ImHDU.__dict__['_info'][key] = ImHDU_template.__dict__['_info'][key] # copy info
            elif key not in templ_info_keys:
                new_ImHDU.__dict__['_info'].__delitem__(key) # delete if not in template
    ...
    def copy_template_BinTable(self, ext_name, cols='all', dtypes=None):
        ...
        nrows = self.fits_template[idx].read_header()['NAXIS2'] # [fitsio]
        if cols=='all':
            cols = self.fits_template[idx].get_colnames() # [fitsio]

        self.HDU_drafts[ext_name] = self.make_HDU_rec_array(nrows, dtypes)
        copy_cols = self.fits_template[idx].read(columns=cols) # [fitsio]
